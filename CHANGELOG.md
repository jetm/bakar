...
- `bakar insights --timing` now supplies an offline dependency source from the graph the run captured, so the critical path renders from a persisted run directory and the concurrency floor `max(CPU floor, critical path)` becomes reachable from the CLI for the first time. The graph is correlated to the run before use rather than trusted for sitting in its directory: consecutive builds of one target produce near-identical graphs, so a mis-correlated one does not look wrong, it looks like an answer. A graph captured outside the run's window is refused with the window named, and every refusal carries its own reason rather than a blank line.
...
- Added `bakar.buildstats`, a reader for bitbake's per-task buildstats tree. bakar previously read this nowhere, so wall-clock was the only dimension available and the CPU floor - the bound that says whether a build is dependency-bound or throughput-bound at all - could not be computed. CPU time is read from the `rusage ru_utime`/`ru_stime` lines rather than the bare `utime`/`stime` ones: a task file carries both, the first in seconds and the second in clock ticks, and reading the wrong pair inflates CPU by roughly two orders of magnitude. Discovery reports `absent`, `empty` and `parsed` as distinct outcomes, because a tree that was never found and a build that recorded nothing both yield zero tasks and mean opposite things.
- `bakar insights` now reports the sstate regime, printed above the timings it qualifies. Two builds of one target in different regimes differ by 65% on this fleet (26.4 min cold against 8.9 min seeded), so every duration in the report is comparable only against another run in the same regime - and a report that omitted it invited exactly the comparison that cannot be made. The data was already captured in `bitbake-events.json` and simply never surfaced. A run whose setscene counters are all zero AND which recorded no tasks reports as *unmeasured* rather than cold, because `eventlog` returns a zero-seeded block verbatim when bitbake's raw log is missing, and reading those zeros as a cold build would record an unmeasured run as a measured one on the success path.
- `bakar sstate-seed` gained `--workspace`, naming the tree the oe-core release is read from. The checkout that owns a seed is not always a bakar workspace - the benchmark tree carries oe-core and none of the workspace markers, and it owns the seed that actually pays off - so without this the one seed worth having could not be named.
