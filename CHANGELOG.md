...
### Added
- Added seven `bakar doctor` checks that verify the host uninative tarball's wiring, all gated on `[build] uninative = true` in host mode on an Arch-family host. `uninative-fragment` (BLOCK) asserts the `yocto-uninative-tarball` fragment is actually installed - the overlay gate keys on that file existing, so a missing package previously dropped the whole wiring without a word and the `require`'s loud parse failure never got a chance to fire. `uninative-glibc` (BLOCK) asserts the fragment's `UNINATIVE_MAXGLIBCVERSION` is at least the buildtools sysroot's glibc, which is the number bounding the version nodes native binaries can emit; below it, those binaries reference nodes the uninative loader cannot resolve. `uninative-checksum` (BLOCK) compares the declared `UNINATIVE_CHECKSUM[x86_64]` against the SHA-256 of the payload in the fragment's own mirror. `uninative-dldir-links` (BLOCK) removes any `<DL_DIR>/uninative` cache entry whose mirrored payload link has gone dangling, clearing the `.done` stamp that would otherwise make uninative skip its fetch and disable itself mid-build. `uninative-mirror-hit` (WARN) reports a payload fetched over the network rather than linked from the local mirror - correct, but no longer offline or reproducible. `uninative-cluster-ceiling` (BLOCK, cluster mode only) publishes this node's ceiling to the shared mount and then compares every peer record, so the node whose ceiling just moved blocks itself instead of poisoning shared sstate.
- Added `bakar doctor --post-build`, which adds post-build checks to the ordinary pre-flight run rather than replacing it, reporting through the same `--json` shape and the same exit-2 rule. It carries `uninative-leak` (BLOCK), which walks a finished build's native work tree for glibc version nodes above the tarball's ceiling, including nodes reached one `DT_NEEDED` edge away through an unsanctioned host library - the case where the artifact's own symbol table reads clean. Off by default because the scan walks the whole native tree, and a pre-flight run has nothing built for it to read.
- Builds now emit structured milestone records to the systemd journal: `build_start`, a `progress` snapshot every `journal_interval` seconds (default 300), `task_failed` at the moment of failure, and `build_end` (including on an abort). Every record carries `BAKAR_RUN_ID`, `BAKAR_MACHINE`, `BAKAR_WORKSPACE` and - when the build is actually scoped - `BAKAR_SCOPE_UNIT`, which joins them to systemd's own `Consumed ... CPU time`, memory-peak and OOM lines for the same unit. A `progress` record adds build state (tasks done/total, percent, sstate coverage, running count, warnings, errors), host health (`LOAD1`, PSI cpu/io/memory, available memory, swap used, free space on the build tmpdir) and cache health (sccache hit rate and verdict, ccache hit rate), so a build that is slow rather than broken can be told apart from one that is idle and stuck, and a cache that quietly stopped hitting is visible. Build **output** is deliberately not sent - it stays in `kas.log` - because journald silently drops messages past its 10,000-per-30s rate limit and a scope unit cannot raise that limit. Records go over journald's native socket, so there is no new dependency and the emitter degrades to a no-op wherever the socket is absent. Configurable via `journal` (default `true`) and `journal_interval`.

### Changed
- `[build] ccache` now defaults to `false` (previously `true`). A yocto-bench.py A/B on core-image-minimal/qemux86-64 found cold-to-warm ccache raised the hit count 65% (3603/42090 -> 5939/42090) but only saved 2% of wall-clock (26.7 -> 26.2 min) against local disk, and bakar's actual topology - an NFS-shared `ccache_dir` - measured 2.3x slower cold-build wall-clock than local disk at a comparable ~8.7% hit rate. An explicit `[build] ccache = true` in `~/.config/bakar/config.toml`, a workspace `.bakar.toml`, or `BAKAR_CCACHE=1` still opts a host back in; the resolution precedence itself is unchanged, only the bottom-of-stack default flipped.

### Fixed
- A repeat `bakar bitbake` of the same target no longer re-parses every recipe. The idle-scope reclaim released the unit name by stopping the scope, which killed bitbake's memory-resident cooker along with its in-memory parse cache - so the second run of a target paid a full re-parse (4,385 recipes, 26s on an i.MX93 tree, far worse on a chromium-scale one) to free a name. That cost only buys something when the run carries cgroup resource controls, because a `MemoryMax` that does not contain the process doing the allocating is not a ceiling; with no controls configured - the default - the scope's remaining jobs are session-survival, which applies to the client being launched, and `oom_score_adj`, which the surviving cooker already inherited at fork. bakar now steps around an idle scope with a run-scoped unit name in the default case and keeps the cooker alive, and stops the scope only when resource controls are configured. A scope holding a live client or worker is untouched in either case, so a genuinely concurrent same-config build still collides. Because the launched unit is no longer always the config-derived one, `BAKAR_SCOPE_UNIT` and the logged journal hint are read back from the launched command rather than re-derived.
- The `uninative-leak` check carried by `bakar doctor --post-build` now follows a declared dependency only into the host library directories, the two sanctioned trees and the native work tree; a dependency is reported as **out of scope** rather than read when at least one of the locations it named lay outside those roots and none of the others resolved it. That is a new, fourth lesser outcome, distinct from a library that was looked for and not found. It closes a path by which a file sitting in the work tree could steer the scan onto an arbitrary readable path on the host and have that path echoed back into the operator's report - the scan followed a location the artifact itself named, so a build output was able to choose what the check went and looked at. A dependency that names a path inside those roots still resolves exactly as before and can still raise a BLOCK, and the confinement tests where a candidate landed rather than how the dependency was spelled - a path-qualified name landing inside a permitted root resolves as it always did. Coverage does narrow, though: a dependency reachable only through a run path pointing outside the permitted roots is no longer read, so a leak that could previously be found only that way now reports as out of scope, which is a WARN rather than a BLOCK. The measured cost on the reference 236-recipe `build-qemuarm64` tree is zero lost resolutions, because refusal is per candidate directory and the one artifact there carrying a foreign run path resolved every dependency on a later candidate under `/usr/lib`. The exposure is host-dependent: it would bite on a host where a library is findable only under a prefix no permitted root covers - a devtoolset, Nix or Homebrew-on-Linux install - since the run path was then the only route that ever reached it. `docs/doctor.md` carries the full rationale.
- Text a report quotes from a build artifact - a path, a recipe name, a declared library name - is now neutralized before it reaches the console, so a directory or library name containing console markup, escape sequences or unbounded text can no longer forge report formatting, rewrite the terminal title, or abort the entire `bakar doctor` run partway through rendering and discard every finding it had already made. Only the printed form is neutralized; the values the scan compares, caches and resolves against stay byte-accurate.
- A named pipe under a recipe's work directory no longer hangs `bakar doctor --post-build` indefinitely. The scan tested each file for the ELF magic by opening it and reading four bytes, and an ordinary `open()` on a FIFO with no writer blocks forever with no timeout to interrupt it - reachable without an attacker, since a recipe or test suite can leave one behind and both `tar` and pseudo preserve FIFOs through sstate. Only regular files are opened now, which also stops sockets and device nodes being read at all.
- The unresolved-dependency warning no longer fires on every healthy build. Measured on the same 236-recipe tree, 188 of 118,549 declared dependencies did not resolve, and any one of them turned this BLOCK-severity check into a WARN - so the warning channel carried no information and a genuine finding had nowhere to stand out. Three classes no longer trigger it, each justified by counted evidence: a library the build itself provides elsewhere under the work tree, where the recorded location merely names a staging path the file is not at and the walk reads that copy on its own (118); an artifact that is not a host-platform ELF and so cannot load under the uninative loader whatever it declares (51); and an artifact that states no glibc version requirement of its own, so it is not the product of a native compile through the buildtools toolchain - every such compile emits at least one glibc node - and an edge it fails to resolve is not evidence about the output this ceiling governs (14). That third one is a judgement about which artifacts the gate is about, not a deduction about their dependencies: an artifact's own empty node set says nothing about what its dependency requires. Three of the 188 still warn, and should; the remaining two are reported out of scope by the confinement. Nothing is hidden - every excluded dependency is still counted and named by class in the verdict on PASS, FAIL and WARN alike, and the narrowing is never applied to a dependency reported out of scope.
- The `uninative-leak` check carried by `bakar doctor --post-build` now counts only the glibc version nodes an artifact *requires*, read from its `DT_VERNEED`. It previously counted every `GLIBC_<maj>.<min>` node the ELF reader printed, including the ones an artifact or a dependency *defines* rather than requires - so on any host whose libc defines nodes above the tarball's ceiling, libc's own definitions were attributed to every artifact linked against it and those artifacts were named as leaking. Nothing about the ceiling or the sanctioned trees changed; what changed is which nodes count as evidence against them, so a recipe that still appears is one whose requirements genuinely exceed the ceiling. The trap that produced the wrong reading is worth stating, since it is not obvious from `objdump`'s output: `objdump -T` parenthesises a version node when the symbol's version binding is non-default, which binutils sets for compat definitions as well as for undefined symbols, so the parenthesis is an upper bound on requirements rather than a measure of them. `DT_VERNEED` holds requirements and nothing else, and reading it retires the `-T` invocation entirely - `objdump -p` alone now supplies the version nodes, the `DT_NEEDED` entries and the `DT_RUNPATH` search path. Reading the block does make the scan locale-sensitive where the old reading was not, so the reader now pins `LC_ALL` and blanks `LANGUAGE`: binutils ships translations for every string the scan matches, and on a French or Spanish desktop an unpinned reader would find none of them, report zero required nodes for every artifact, and return a BLOCK-severity all-clear over a tree it never understood. Two further changes to the same scan: a walk that completes having read no dynamically linked native artifact now reports SKIP rather than a verdict, because a BLOCK-severity all-clear over zero evidence claims more than the scan saw (the message names `rm_work` as the usual cause, since it deletes each recipe's work directory as that recipe finishes, and the class can be inherited from the distro or `local.conf` rather than set through bakar; relocatable objects and static binaries do not count towards that floor, since neither states a requirement, so a stripped tree retaining one leftover `build/foo.o` no longer reads as evidence); and the tree is traversed in sorted order at each directory level, so the same tree always yields the same findings and the truncated report always names the same artifacts instead of whichever ones the filesystem happened to return first.
- The live build panel no longer stacks a fresh copy of itself on every refresh after a task fails. On the first failure the UI stops the Rich `Live` so the collapsed frame lands in scrollback above the streaming error text, then restarts it once the failure context has landed. Rich's `Live.stop()` sets `vertical_overflow="visible"` so its final frame renders uncropped and never restores the previous value, so after the restart every frame rendered uncropped and Rich's cursor-up erase - sized for a panel taller than the terminal - overshot and left the old frame on screen. The freeze now carries the value across the stop and restores it on restart. Only builds that reached a failed task were affected; a clean build never freezes the panel.
