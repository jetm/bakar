...
### Added
- Added the `bakar feed` command surface: `sync`, `index`, `serve`, `stop`, `status`, `gc` and `doctor`. `sync` stages a finished build and renders every repository its `avocado-repo.map` declares, at both the head path and under `snapshots/<id>/`, writing the snapshot pointer last so an interrupted sync leaves the previous pointer rather than announcing a snapshot whose repositories are missing. `serve` binds loopback: `python -m http.server` binds every interface when given no `--bind`, which was measured publishing the whole feed root - packages, snapshots and the state directory - to anything that could reach the host, while the CLI printed `http://localhost`. It also records the port it bound, because the port is an argument and `status` has no other way to know it, and it confirms the server by connecting rather than by `Popen` returning, since output goes to `DEVNULL` and a port collision is otherwise invisible.
- Added `bakar feed doctor`, and the same check now runs at the head of every `sync`. The feed is the one part of bakar that depends on things pip cannot install: `render-pool-local.py` shells straight out to `createrepo_c` with no `which` guard and no fallback, and `repo-stage-rpms.sh` is bash driving a `tar` pipe. Every prerequisite is checked together and reported as a list rather than one exception at a time, because a first run on a fresh machine typically fails several of them - an interpreter, a native binary, two shell tools, a layer checkout and a finished build - and the `createrepo_c` package is spelled differently on every distro and does not exist on macOS, so the hint names the package rather than the command.
- `bakar feed doctor` and `sync` also check that `--release`/`--channel` match the build's `DISTRO_CODENAME`. That variable IS `release/channel` - it is what dnf expands `$releasever` to, so it decides the path a client composes. bakar's defaults are `2024/edge`, which is right for some builds and wrong for others, and the wrong pair renders a perfectly valid feed at a path nothing will ever ask for, with every symptom appearing client-side minutes later. Skipped rather than guessed when the build records no codename with a separator in it.
- Added `feed_retention.py` and `bakar feed gc`. Keeps the head plus the N newest snapshots, retains every pinned snapshot on top of that count, and removes a pool entry only when no retained repository's live metadata resolves to it - scanned across the whole feed, since a repository in another release can resolve into this channel's pool. Deletion stays inside the channel the caller named, so `gc --channel edge` cannot mutate a channel the invocation never mentioned. Previews by default, and every confirmed run audits itself for dangling references afterwards. It also sweeps stale repository metadata: the renderer writes a new checksum-named `primary`/`other`/`filelists` triple on every render and rewrites `repomd.xml` to name it without removing the previous one, so a re-rendered repository keeps every generation it ever had - measured at 14 metadata files of which 3 were live.

### Fixed
- The snapshot pointer is now written per machine at `target/<machine>/snapshots-latest.json`, which is where its only consumer looks. avocado-cli composes the URL as `{release}/{channel}/target/<machine>/snapshots-latest.json`; a single pointer at the channel root is not a near-miss but invisible, because avocado-cli treats a 404 there as "feed without snapshots" and falls back to tracking the live head while recording no pin at all - so snapshot pinning appeared to work and pinned nothing. Per machine is also the only correct shape: each snapshot holds only the repositories from the sync that made it, so one channel-wide pointer names a snapshot that most machines are absent from and a client following it 404s mid-resolve. Retention therefore reads every pointer and treats all of them as pins - a target nobody has rebuilt still pins an older snapshot, and dropping it would break that one target while every other machine kept working.
- Unreadable repository metadata now refuses rather than reading as "references nothing". A repository whose index parses but whose `primary.xml.gz` cannot be decompressed still serves that index, so treating its packages as unreferenced orphans deleted content the feed was actively publishing - and the post-run audit, reading the same broken file, reported clean. `gzip.BadGzipFile` subclasses `OSError` and was swallowed by an ordinary handler; `zlib.error` does not subclass `OSError` and aborted the run outright. Both are caught now, both suppress pool reclamation for the whole run, and both are reported.
...
[Unreleased]: https://github.com/jetm/bakar/compare/v0.26.0...HEAD
