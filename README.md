...
- **Pre-flight diagnostics** - ~30 host/container/workspace checks with PASS/WARN/BLOCK gating and PSI throttle calibration. The host-environment thresholds (inotify instances/watches, swappiness ceiling, docker nofile floor, minimum memory) are configurable via `host.*` settings keys; defaults equal the values doctor previously hardcoded. See [docs/doctor.md](docs/doctor.md), [docs/settings.md](docs/settings.md).
