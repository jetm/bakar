...
| `scope_cpu_weight` | int | `50` | `CPUWeight` for the scope, below the systemd default of `100` so the host stays responsive under contention. Only bites when something else wants the CPU, so it never slows an otherwise-idle build. Range `[0, 10000]`; `0` omits the property. |
| `scope_io_weight` | int | `50` | `IOWeight` for the scope, below the default of `100`. Same contention-only semantics as `scope_cpu_weight` (requires the io controller delegated to the user manager to take effect). Range `[0, 10000]`; `0` omits the property. |
