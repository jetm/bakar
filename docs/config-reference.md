...
| `scope_memory_high` | float | `0.85` | Fraction of physical RAM at which the scope's `MemoryHigh` soft reclaim throttle kicks in. Sits just below `scope_memory_max` so a legitimately RAM-hungry build runs unthrottled until it nears the hard ceiling (mostly reclaiming cold page cache, not the working set). Must be in `(0, 1]`. |
| `scope_memory_max` | float | `0.90` | Fraction of physical RAM for the scope's `MemoryMax` hard ceiling. Kept **below** total RAM so a memory runaway is OOM-killed inside the build cgroup, leaving headroom for the kernel, PID 1, and the desktop instead of taking the whole box down. Must be in `(0, 1]`. |
