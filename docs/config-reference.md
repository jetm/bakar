...
| `scope` | bool | `true` | Run the build inside a transient `systemd-run --user --scope` so it survives terminal/session teardown (SSH disconnect, closed shell) and runs under a cgroup memory ceiling. The five `scope_*` keys below tune the scope's resource controls. Container-mode builds still get session-survival, but the memory ceiling only bounds the `kas-container`/`docker` client, not the container itself (the container lives in the runtime's cgroup). No-op when `systemd-run`/the user manager is unavailable. Opt out per-invocation with the global `--no-scope`. **Does not** touch `BB_NUMBER_THREADS`/`PARALLEL_MAKE`. |
| `scope_memory_high` | float | `0.0` | Fraction of physical RAM for the scope's `MemoryHigh` soft reclaim throttle. **Off by default** (`0.0` = omit). Enable only on a *dedicated* build host: on a host with a large zram/zswap swap, `MemoryHigh` reclaim on the build's anon-heavy (unswappable) working set spins CPUs in direct reclaim, which swap-thrashes the desktop and - with `softlockup_panic=1` - can panic the box. Range `[0, 1]` (0 disables). |
| `scope_memory_max` | float | `0.0` | Fraction of physical RAM for the scope's `MemoryMax` hard ceiling. **Off by default** (`0.0` = omit) for the same reason: `memory.max` caps only RAM-resident memory, so with a large zram swap the build spills into swap (stored compressed in RAM) instead of OOMing and the "ceiling" never bounds physical RAM. Enable only on a dedicated build host where OOM-killing the build to protect the host is wanted; when set, the scope also emits `MemorySwapMax=0` so the cap becomes a real RAM ceiling. Range `[0, 1]` (0 disables). |
