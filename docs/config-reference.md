...
| `ccache` | bool | `false` | Enable ccache. Mutually exclusive with `sccache_dist` - chaining the two launchers double-wraps `CC`, so enabling `sccache_dist` removes ccache for that build. |
