...
def test_resolve_qcom_defaults(tmp_path: Path) -> None:
    cfg = resolve(ResolveRequest(workspace=_workspace(tmp_path), bsp_family="qcom"))
...
def test_resolve_qcom_bsp_root_is_workspace_qcom(tmp_path: Path) -> None:
    ...
    cfg = resolve(ResolveRequest(workspace=ws, bsp_family="qcom"))
...
def test_resolve_non_qcom_build_dir_name_is_build(tmp_path: Path, family: str) -> None:
    ...
    cfg = resolve(ResolveRequest(workspace=tmp_path, bsp_family=family))  # type: ignore[arg-type]
