...
def test_load_baselines_single_sample_zero_stddev(tmp_path: Path) -> None:
    f = tmp_path / "t.json"
    f.write_text(
        json.dumps({"tasks": {"do_compile": {"count": 1, "mean": 10.0, "m2": 0.0}}}),
        encoding="utf-8",
    )
    baselines = task_timings.load_baselines(f)
    assert baselines == {"do_compile": (10.0, 0.0)}
...
def test_load_baselines_sample_stddev(tmp_path: Path) -> None:
    ...
    f = tmp_path / "t.json"
    f.write_text(
        json.dumps({"tasks": {"do_compile": {"count": 2, "mean": 15.0, "m2": 50.0}}}),
        encoding="utf-8",
    )
    mean, stddev = task_timings.load_baselines(f)["do_compile"]
    assert mean == 15.0
...
def test_update_from_events_creates_file_and_parent(tmp_path: Path) -> None:
    ...
    _write_events(events, [{"task": "do_compile", "started": 100.0, "completed": 130.0}])
...
def test_update_from_events_skips_missing_and_negative(tmp_path: Path) -> None:
    ...
    _write_events(
        events,
        [
            {"task": "do_compile", "started": 100.0, "completed": 130.0},
            {"task": "do_install", "started": None, "completed": 5.0},
            {"task": "do_fetch", "started": 5.0, "completed": None},
            {"task": "do_clock_skew", "started": 200.0, "completed": 190.0},
    ...
    data = json.loads(timings.read_text(encoding="utf-8"))
    assert set(data["tasks"]) == {"do_compile"}
...
def test_update_from_events_accumulates_across_calls(tmp_path: Path) -> None:
    ...
    for completed in (110.0, 130.0):
        events = tmp_path / "events.json"
        _write_events(events, [{"task": "do_compile", "started": 100.0, "completed": completed}])
        task_timings.update_from_events(events, timings)

    baselines = task_timings.load_baselines(timings)
    mean, stddev = baselines["do_compile"]
...
def test_update_from_events_tolerates_malformed_existing(tmp_path: Path) -> None:
    ...
    events = tmp_path / "events.json"
    _write_events(events, [{"task": "do_compile", "started": 100.0, "completed": 110.0}])

    task_timings.update_from_events(events, timings)

    data = json.loads(timings.read_text(encoding="utf-8"))
    assert data["tasks"]["do_compile"]["count"] == 1
