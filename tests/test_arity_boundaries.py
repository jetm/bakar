...
Later tasks repack internal helpers (``_graceful_wait``,
``_render_sstate_lines``, ``_run_pty_with_ui``, ``config.resolve``); extend
...
REPACKED: list[Case] = [
    Case("bakar.commands._app", "_main", "_main_impl", "_GlobalOptions", 12),
    Case("bakar.commands.clean_cache", "clean_cache", "_clean_cache_impl", "_CleanCacheCtx", 10),
    Case("bakar.commands.getvar", "getvar", "_getvar_impl", "_GetvarCtx", 10),
    Case("bakar.commands.sync", "sync", "_sync_impl", "_SyncCtx", 10),
    Case("bakar.commands.stress_parse", "stress_parse", "_stress_parse_impl", "_StressParseCtx", 11),
]
...
def test_impl_takes_exactly_its_context(case: Case) -> None:
    mod = importlib.import_module(case.module)
    params = list(inspect.signature(getattr(mod, case.impl_name)).parameters.values())

    assert len(params) == 1, f"{case.impl_name} takes {len(params)} parameters, expected 1"
    ...
    assert annotation in (case.ctx_name, getattr(mod, case.ctx_name)), (
        f"{case.impl_name} takes {annotation!r}, expected {case.ctx_name}"
    )
