...
from bakar.config import ResolveRequest, compose_preset_output_path, resolve
...
def test_hashserv_state_key_shared_across_workspaces_same_sstate(tmp_path, monkeypatch) -> None:
    ...
    cfg_a = resolve(ResolveRequest(workspace=tmp_path / "wsA", bsp_family="nxp", user_config=uc))
    cfg_b = resolve(ResolveRequest(workspace=tmp_path / "wsB", bsp_family="nxp", user_config=uc))
...
def test_resolve_preset_beats_user_config(tmp_path) -> None:
    ...
    cfg = resolve(ResolveRequest(workspace=_workspace(tmp_path), bsp_family="nxp", preset=preset, user_config=uc))
...
def test_resolve_explicit_spec_beats_preset(tmp_path) -> None:
    ...
    cfg = resolve(ResolveRequest(workspace=_workspace(tmp_path), bsp_family="nxp", spec=spec, preset=preset))
...
def test_resolve_stop_on_error_user_config_disables(tmp_path) -> None:
    ...
    cfg = resolve(
        ResolveRequest(workspace=_workspace(tmp_path), bsp_family="nxp", user_config=UserConfig(stop_on_error=False))
    )
...
def test_resolve_stop_grace_seconds_user_config_overrides(tmp_path) -> None:
    ...
    cfg = resolve(
        ResolveRequest(workspace=_workspace(tmp_path), bsp_family="nxp", user_config=UserConfig(stop_grace_seconds=45))
    )
...
def test_resolve_ccache_user_config_disables(tmp_path) -> None:
    ...
    cfg = resolve(
        ResolveRequest(workspace=_workspace(tmp_path), bsp_family="nxp", user_config=UserConfig(ccache=False))
    )
...
def test_resolve_ccache_user_config_enables(tmp_path) -> None:
    ...
    cfg = resolve(ResolveRequest(workspace=_workspace(tmp_path), bsp_family="nxp", user_config=UserConfig(ccache=True)))
...
def test_resolve_rm_work_user_config_enables(tmp_path) -> None:
    ...
    cfg = resolve(
        ResolveRequest(workspace=_workspace(tmp_path), bsp_family="nxp", user_config=UserConfig(rm_work=True))
    )
...
def test_resolve_sccache_dist_user_config_toggles(tmp_path) -> None:
    ...
    on = resolve(
        ResolveRequest(workspace=_workspace(tmp_path), bsp_family="nxp", user_config=UserConfig(sccache_dist=True))
    )
    off = resolve(
        ResolveRequest(workspace=_workspace(tmp_path), bsp_family="nxp", user_config=UserConfig(sccache_dist=False))
    )
...
def test_resolve_sccache_dist_override_flag_resolves_ccache_true(tmp_path) -> None:
    ...
    cfg = resolve(ResolveRequest(workspace=_workspace(tmp_path), bsp_family="nxp", sccache_dist_override=True))
...
def test_resolve_omitted_bsp_family_takes_preset_family(tmp_path) -> None:
    ...
    cfg = resolve(ResolveRequest(workspace=_workspace(tmp_path), preset=preset))
...
def test_resolve_omitted_bsp_family_defaults_to_nxp_without_preset(tmp_path) -> None:
    ...
    cfg = resolve(ResolveRequest(workspace=_workspace(tmp_path)))
...
def test_resolve_explicit_bsp_family_conflicting_with_preset_raises(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="ti-test-preset") as excinfo:
        resolve(ResolveRequest(workspace=_workspace(tmp_path), bsp_family="nxp", preset=preset))
...
def test_resolve_explicit_bsp_family_matching_preset_succeeds(tmp_path) -> None:
    ...
    cfg = resolve(ResolveRequest(workspace=_workspace(tmp_path), bsp_family="ti", preset=preset))
...
def test_resolve_fallback_bsp_family_conflicting_with_preset_defers_silently(tmp_path) -> None:
    ...
    cfg = resolve(
        ResolveRequest(workspace=_workspace(tmp_path), bsp_family="nxp", preset=preset, family_is_explicit=False)
    )
